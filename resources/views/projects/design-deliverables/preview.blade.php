...
<main>
    <header class="header">
        <div>
            <p class="eyebrow">Design Deliverable</p>
            <h1>Preview PDF Mockup</h1>
            <p class="project">{{ $project->code }} - {{ $project->name }} - {{ $task->title }}</p>
        </div>
        <nav class="actions" aria-label="Aksi dokumen">
            <a href="{{ route('projects.show', $project) }}#workspace" class="button">Kembali ke Project</a>
            <a href="{{ $downloadUrl }}" class="button primary">Download PDF</a>
        </nav>
    </header>
    ...
    <section class="card">
        <div class="details">
            <div>
                <p class="name">{{ $deliverable->title }}</p>
        ...
        <div class="viewer">
            @if ($pdfBase64)
                ...
                <div id="pdf-error" class="warning hidden">Preview PDF tidak dapat dibuat. Gunakan Download PDF untuk membuka file secara manual.</div>
                <iframe id="pdf-frame" class="hidden" title="Preview PDF Mockup"></iframe>
            ...
            @else
                <div class="warning">{{ $previewMessage ?: 'Preview tidak dapat dimuat otomatis. Gunakan tombol Download PDF untuk membuka file secara manual.' }}</div>
            @endif
        </div>
    </section>
</main>
